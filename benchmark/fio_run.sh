...
for ((n = 0; n <= 200; n += 20)); do
    ...
    for i in {1..10}; do
        ...
        sudo bash -c "sync; echo 3 > /proc/sys/vm/drop_caches"
        fio  --filename="$MOUNT_POINT_EN"/file"$n".txt --readwrite=read --ioengine=psync --invalidate=1 --readonly --name=read_seq --bs=4k  --iodepth=1 --size="$TEST_FILE_SIZE" --output="$RESULTS_DIR/all_sread_"$i"_file_"$n".txt"

        
        
        # Random read test   
        sudo bash -c "sync; echo 3 > /proc/sys/vm/drop_caches"
        fio  --filename="$MOUNT_POINT_EN"/file"$n".txt --readwrite=randread --ioengine=psync --invalidate=1 --readonly --name=read_rand --bs=4k  --iodepth=1 --size="$TEST_FILE_SIZE" --output="$RESULTS_DIR/all_rread_"$i"_file_"$n".txt"
