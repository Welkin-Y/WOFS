...
class Meta {
    ...
    time_t lastModified;
public:
    Meta(std::string name, size_t start, size_t size, unsigned int permission, unsigned int owner, unsigned int group, bool isDir, size_t num_blocks, time_t lastModified);
    std::string getName();
    size_t getStart();
    size_t getSize();
